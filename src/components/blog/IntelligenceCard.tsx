...
export default function IntelligenceCard({ article, featured = false }: { article: Article, featured?: boolean }) {
    if (featured) {
        return (
            <div className="group relative w-full overflow-hidden rounded-[2.5rem] border border-stone-200 shadow-2xl bg-white">
                ...
                <Link href={`/insights/${article.slug}`} className="absolute inset-0 z-0" aria-label={`Read ${article.title}`} />

                <div className="relative z-10 p-8 md:p-12 flex flex-col justify-end min-h-[500px] pointer-events-none">
                    <div className="flex items-center gap-3 mb-4 flex-wrap">
                        <span className="px-3 py-1 rounded-full bg-primary text-onyx text-xs font-bold uppercase tracking-wider">
                    ...
                    <div className="mt-8 pointer-events-auto">
                        <Link href={`/insights/${article.slug}`} className="inline-flex items-center gap-2 text-primary font-bold group-hover:gap-4 transition-all">
                            Read Full Briefing <ArrowRight size={20} />
                        </Link>
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="group relative flex flex-col h-full bg-white border border-stone-200 rounded-[2.5rem] overflow-hidden hover:border-primary/50 hover:shadow-2xl hover:shadow-stone-200/50 transition-all duration-300">
            ...
            <div className="p-6 md:p-8 flex flex-col flex-1 relative z-10 pointer-events-none">
                ...
                <p className="text-stone-500 text-sm md:text-base leading-relaxed mb-6 flex-1 italic font-medium">
                    &quot;{article.excerpt}&quot;
                </p>
